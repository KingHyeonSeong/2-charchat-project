...
from flask import request  # POST 데이터에서 username 추출 위해 추가
import warnings
...
def generate_response(user_message):
    ...
    if user_message.strip().lower() == "init":
        initial_reply = f"""안녕, 너가 신입생 {request.get_json().get("username", "").strip()}이구나. 오늘 첫 수업은 어땠어?"""
        try:
...
[사용자 이름: {request.get_json().get("username", "").strip()}]
"""
...
"""
    ...
    raw_response = langchain_response["text"].strip()
